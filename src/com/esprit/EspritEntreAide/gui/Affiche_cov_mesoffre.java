 ...
 * @author me
 */
public class Affiche_cov_mesoffre extends BaseForm{
    
    Form f = new Form("Mes Offres",BoxLayout.y());
    SpanLabel lb;
        ...
        Label lbdesiption = new Label(e.getDescription());
         Button btsupp = new Button("supp");
          btsupp.addActionListener((l)->{
            Affiche_cov_mesoffre c = new Affiche_cov_mesoffre(theme);
            c.supp_cov_offre(e);
            Affiche_cov_mesoffre c2 = new Affiche_cov_mesoffre(theme);
            c2.show();
           });
    ...
    public Affiche_cov_mesoffre(Resources res) {
...
//          });
          List<Covoiturage> listcovs=new ArrayList<>();
           listcovs=servicecov.getListmesoffrecov();
            for(Covoiturage c: listcovs){
               add(addItem(c));
          }
          
    }
    public void supp_cov_offre(Covoiturage cov)
    {
          ServiceCov servicecov=new ServiceCov();
          servicecov.SuppCovOffre(cov);  
    }
